...
function end(chunk, encoding) {
  if (chunk !== undefined) this.write(bufferify(chunk, encoding), encoding);
  this.close();
  return true;
}
...
WorkerSocket.prototype.connect = function(source, callback) {
  ...
  function consume() {
    return ch.consume(source, function(msg) {
      ...
      // Self.push(msg && msg.content);
      self.push(bufferify(JSON.stringify(msg)));
  ...
  function consume() {
    return ch.consume(source, function(msg) {
      if (msg !== null) {
        ...
        // Push message with all properties instead of just content
        self.push(bufferify(JSON.stringify(msg)));
      } else self.push(null);
    }, {noAck: false}).then(function(ok) {
      self.consumers[source] = ok.consumerTag;
...
RepSocket.prototype.write = function(chunk, encoding, corrId) {
  var ch = this.ch,
      current = corrId ?
        _.head(this.requests.splice(_.findIndex(this.requests, ['properties.correlationId', corrId]), 1)) :
        this.requests.shift();
